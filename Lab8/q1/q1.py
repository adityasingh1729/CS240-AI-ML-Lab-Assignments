...
class RNN:
    ...
    def backward(self,x, a, y_preds, targets):
        ...
        for t in reversed(range(seq_length)):
            ...
            da_next = np.dot(self.Waa.T, daraw)
    ...
    def loss(self, y_preds, targets):
        ...
        loss = np.sum(-np.log(y_preds[t][targets[t]]) for t in range (len(targets)))
    ...
    def predict(self, start):
        ...
        while (counter < max_chars):
            a_prev = np.tanh(np.dot(self.Wax, x) + np.dot(self.Waa, a_prev) + self.ba)
            y = self.softmax(np.dot(self.Wya, a_prev) + self.by)
            idx = np.random.choice(range(self.vocab_size), p=y.ravel())
            txt += self.data_generator.idx_to_char[idx]
            x = np.zeros((self.vocab_size, 1))
            x[idx] = 1
            if idx == newline_character or counter == max_chars - 1:
                break
            counter += 1
