...
def gini_index(classes):
    ...
    Args:
        classes (numpy.ndarray): numpy array of labels
    ...
    gini_impurity_value = 1 - np.sum(probs**2)
...
def entropy(classes):
    ...
    Returns:
        float: A float value representing the entropy of the set of labels.  
    """
    #### Student Code Start : TASK 1
    entropy_value = None
    ...
    entropy_value = -np.sum(probs * np.log2(probs + 1e-12))
...
def information_gain(examples, classes, attr):
    ...
    Args:
        examples (numpy.ndarray): A numpy array of shape containing input examples.
    ...
    # labels, counts = np.unique(examples.t[attr], return_counts = True)
    attribute_values, attribute_counts = np.unique(examples[:, attr], return_counts=True)

    # Calculate the entropy of the entire dataset before splitting
    entropy_original = entropy(classes)

    entropy_after_split = 0
    for value, count in zip(attribute_values, attribute_counts):
        # Get the indices of examples where the attribute has the current value
        indices = np.where(examples[:, attr] == value)[0]

        # Get the classes corresponding to these indices
        classes_subset = classes[indices]

        # Calculate the weighted entropy of this subset
        weight = count / len(examples)
        entropy_after_split += weight * entropy(classes_subset)

    # Calculate the information gain
    information_gain_value = entropy_original - entropy_after_split
...
def gini_gain(example, classes, attr):
    ...
    Args:
        example (numpy.ndarray): A numpy array of shape containing input examples.
        classes (numpy.ndarray): A numpy array of shape (number_of_examples) containing target labels.
        attr (int): An integer specifying the attribute to split on, ranging from 0 to (number_of_features - 1) inclusive.
    ...
    # Get unique values and their counts for the specified attribute
    attribute_values, attribute_counts = np.unique(example[:, attr], return_counts=True)

    # Calculate the gini index of the entire dataset before splitting
    gini_original = gini_index(classes)

    gini_after_split = 0
    for value, count in zip(attribute_values, attribute_counts):
        # Get the indices of examples where the attribute has the current value
        indices = np.where(example[:, attr] == value)[0]

        # Get the classes corresponding to these indices
        classes_subset = classes[indices]

        # Calculate the weighted gini index of this subset
        weight = count / len(example)
        gini_after_split += weight * gini_index(classes_subset)

    # Calculate the gini gain
    gini_gain_value = gini_original - gini_after_split
    ...
    def __build_tree__(self, features, classes, attributes, depth=0):
        ...
        if (len(attributes) == 0):
            ...
            for attr in attributes:
                check = information_gain(features, classes, attr)
                if (maxInfo < check):
                    maxInfo = check
                    best_attr = attr

            # Get unique values of the best attribute for splitting.
            attribute_values = np.unique(features[:, best_attr])
            ...
            for value in attribute_values:
                indices = np.where(features[:, best_attr] == value)[0]
                subset_features = features[indices]
                subset_classes = classes[indices]
                children[value] = self.__build_tree__(subset_features, subset_classes, remaining_attributes, depth + 1)
