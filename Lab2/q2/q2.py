...
def w_closed_form(X, Y):
    ...
    X = torch.tensor(X)
    w_closed = torch.round(torch.mm(torch.linalg.inv(torch.mm(X.t(), X)), torch.mm(X.t(), Y)) * 10000) / 10000
...
def transform_features(X, degree=1):
    ...
    for d in range(1, degree + 1):
        phi_X = np.hstack((phi_X, np.power(X, d)))
    # Student code end
...
def l2_loss(X, Y, w):
    ...
    X = torch.tensor(X)
    vec = Y - torch.mm(X, w)
    loss = np.float64((torch.mm(vec.t(), vec).item())/n)
    # Student code end
...
def l2_loss_derivative(X, Y, w):
    ...
    # Please take care of normalization factor 1/n
    n, _ = X.shape
    derivative = (2 / n) * (torch.mm(torch.mm(X.t().double(), X.double()), w.double()) - torch.mm(X.t(), Y))
    # Student code end
...
if __name__ == '__main__':
    ...
    for degree in possible_degrees:
        ...
        print(f"{degree} {l2_loss_train} {l2_test_loss}")
        if (l2_test_loss) < min_loss:
            min_loss = l2_test_loss
