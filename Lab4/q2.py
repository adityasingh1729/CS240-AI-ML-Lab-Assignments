...
class MultiClassLogisticRegression:
    ...
    def predict(self, X):
        ...
        # print("Done")
        pre_vals = np.dot(X, np.matrix.transpose(self.weights))
    #   ## YOUR CODE ENDS HERE ###
    ...
    def softmax(self, z):
        ...
        # print(z.shape)
        # print(self.weights.shape)
        post_softmax = np.exp(z) / np.sum(np.exp(z), axis=1, keepdims=True)
        ### YOUR CODE ENDS HERE ###
        return post_softmax
    ...
    def cross_entropy(self, y, probs):
        ...
        ### YOUR CODE BEGINS HERE ###
        post_cross_entropy = -np.mean(np.sum(y * np.log(probs), axis=1))
        # m = y.shape[0]
        # K = self.weights.shape[0]
        # print(f'hello {self.weights.shape} {y.shape} {probs.shape} {X.shape}')
        # for i in range(m):
        #     for k in range(K):
