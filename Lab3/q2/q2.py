...
def lasso_loss(X, Y, w, lambd):
    ...
    n, d = X.shape
    # loss = np.linalg.norm(Y - np.dot(X, w))
    # loss = np.mean((Y - np.dot(X, w))**2) + lambd*np.linalg.norm(w, ord=1)
    loss = torch.mean((Y - torch.mm(X, w))**2) + lambd * torch.norm(w, p = 1)
    ### YOUR CODE ENDS HERE ###
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    ### YOUR CODE BEGINS HERE ###
    n, d = X.shape
    # derivative = (2/n) * torch.mean(Y - torch.mm(X,w)) + lambd*torch.sign(w)
    derivative = (2/n) * (torch.mm(X.t(), torch.mm(X, w)) - torch.mm(X.t(), Y)) + lambd*torch.sign(w)
    ### YOUR CODE ENDS HERE ###
