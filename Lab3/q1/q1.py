...
def create_weights(data_dictionary, lambda_val):
    ...
    # weights = np.dot(np.linalg.inv(np.dot(X, np.matrix.transpose(X)) + lambda_val*np.identity(m)), np.dot(X, np.matrix.transpose(y)))
    weights = np.dot(np.linalg.inv(np.dot(np.matrix.transpose(X), X) + lambda_val*np.identity(n)), np.dot(np.matrix.transpose(X), y))
    # END TODO
...
def generate_test_error(data_dictionary, weights):
    ...
    test_error = np.mean((y - np.dot(X, weights))**2)
...
def find_optimal_lambda(error_array, lambda_array):
    ...
    minIndex = np.argmin(error_array)
    optimal_lambda = lambda_array[minIndex]
