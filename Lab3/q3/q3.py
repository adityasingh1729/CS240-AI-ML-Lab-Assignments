...
def gradient_descent(X, y, eta, max_steps):
    ...
    while (step < max_steps):
        dw = (2 / n) * (np.dot(np.dot(np.matrix.transpose(X), X), w) - np.dot(np.matrix.transpose(X), y))
        w = w - eta * dw
        w_values.append(w)
        step += 1
...
def stochastic_gradient_descent(X, y, eta, max_steps):
    ...
    step = 0
    while (step < max_steps):
        x_step = X[step].reshape(d, 1)
        y_step = y[step]
        # dw = (2 / n) * (np.dot(np.dot(np.matrix.transpose(x_step), x_step), w) - np.dot(np.matrix.transpose(x_step), y_step))
        dw = 2 * (np.dot(np.matrix.transpose(w), x_step) - y_step) * x_step
        w = w - eta * dw
        w_values.append(w)
        step += 1
